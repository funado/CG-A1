...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->LoadShaderPartFromFile("shaders/frag_blinn_phong_textured_rl.glsl", GL_FRAGMENT_SHADER);
		shader->Link();

		glm::vec3 lightPos = glm::vec3(0.0f,-5.0f, 7.0f);
		glm::vec3 lightCol = glm::vec3(0.9f, 0.85f, 0.5f);
		float     lightAmbientPow = 0.0f;
		...
		//shader->SetUniform("u_Rim", (int)rim);
		//shader->SetUniform("u_VM", glm_vec4);
		...
		imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("Scene Level Lighting Settings"))
			{
				...
				if (ImGui::SliderFloat("Fixed Ambient Power", &ambientPow, -10.00f, 1.0f)) {
					shader->SetUniform("u_AmbientStrength", ambientPow);
				}
			}

			//Imgui Tests
			if (ImGui::Checkbox("No Lighting", &none))
			{
				if (none)
				{
					shader->SetUniform("u_None", (int)none);
					...
					//shader->SetUniform("u_LightAttenuationLinear", 1.0f);
					//shader->SetUniform("u_LightAttenuationQuadratic", 1.0f);
				}
				else
				{
					shader->SetUniform("u_None", 0.0f);
					shader->SetUniform("u_AmbientStrength", 1.0f);
				}
			}
			if (ImGui::Checkbox("Ambient Lighting", &amb))
			{
				if (amb)
				{
					shader->SetUniform("u_AmbientStrength", 1.0f);
					shader->SetUniform("u_AmbientLightStrength", 0.8f);
					shader->SetUniform("u_SpecularLightStrength", 0.0f);
					shader->SetUniform("u_LightAttenuationLinear", 0.0f);
					shader->SetUniform("u_LightAttenuationQuadratic", 0.0f);
			...
			if (ImGui::Checkbox("Specular Lighting", &spec))
			{
				if (spec)
				{
			...
			if (ImGui::Checkbox("Ambient + Specular", &ambSpec))
			{
				if (ambSpec)
				{
					shader->SetUniform("u_AmbientStrength", 1.0f);
			...
			if (ImGui::Checkbox("Ambient + Specular + ", &rim))
			{
				if (rim)
				{
				...
				else
				{
					...
					//shader->SetUniform("u_Toon", 0.0f);
				}
			}
			//if (ImGui::Checkbox("Ambient + Specular + ", &toon))
			//{
			//	if (toon)
			//	{
			//		shader->SetUniform("u_AmbientStrength", 0.5f);
			...
			//		shader->SetUniform("u_SpecularLightStrength", 1.0f);
			//
			//		shader->SetUniform("u_Toon", (int)toon);
			//	}
			//	else
			//	{
			//		shader->SetUniform("u_AmbientStrength", 0.0f);
		...
		Texture2D::sptr diffuse = Texture2D::LoadFromFile("images/metal.jpg");
		...
		GameObject obj2 = scene->CreateEntity("spikeBall");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Spike Ball.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0);
			obj2.get<Transform>().SetLocalPosition(5.0f, 0.0f, 0.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}
		GameObject obj3 = scene->CreateEntity("spikeBall2");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Spike Ball.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0);
			obj3.get<Transform>().SetLocalPosition(5.0f, -5.0f, 0.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj3);
		}
		GameObject obj4 = scene->CreateEntity("spikeBall3");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Spike Ball.obj");
			obj4.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0);
			obj4.get<Transform>().SetLocalPosition(-5.0f, 0.0f, 0.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj4);
		}
		GameObject obj5 = scene->CreateEntity("spikeBall4");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Spike Ball.obj");
			obj5.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0);
			obj5.get<Transform>().SetLocalPosition(-5.0f, -5.0f, 0.0f);
			BehaviourBinding::Bind<SimpleMoveBehaviour>(obj5);
		}

		//Non Moving one, use for lighting examples
		GameObject obj6 = scene->CreateEntity("spikeBall5");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Spike Ball.obj");
			obj6.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0);
			obj6.get<Transform>().SetLocalPosition(0.0f, 0.0f, -1.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj6);
		}
		//Non Moving one, use for lighting examples
		GameObject obj7 = scene->CreateEntity("sub");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/sub.obj");
			obj7.emplace<RendererComponent>().SetMesh(vao).SetMaterial(reflectiveMat);
			obj7.get<Transform>().SetLocalPosition(0.0f, 0.0f, -1.0f);

			obj7.get<Transform>().SetLocalPosition(-2.0f, 0.0f, -5.0f);
			obj7.get<Transform>().SetLocalScale(glm::vec3(0.05f));
			obj7.get<Transform>().SetLocalRotation(glm::vec3(0.0f,0.0f,180.0f));

			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(obj7);

			pathing->Points.push_back({ -4.0f, -4.0f, -5.0f });
			pathing->Points.push_back({ 4.0f, -4.0f, -5.0f });
			pathing->Points.push_back({ 4.0f,  4.0f, -5.0f });
			pathing->Points.push_back({ -4.0f,  4.0f, -5.0f });
			pathing->Speed = 2.0f;
		...
		//GameObject obj8 = scene->CreateEntity("moving_box");
		//{
		//	// Build a mesh
		...
		//	
		//	obj8.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0);
		//	obj8.get<Transform>().SetLocalPosition(-2.0f, 0.0f, 1.0f);
		//
		...
		while (!glfwWindowShouldClose(window)) {
			...
			//Do Stuff Here, Transforms/Rotations

			//Left
			obj2.get<Transform>().RotateLocal(0.0f, -45.0f * time.DeltaTime, 0.0f);
			obj3.get<Transform>().RotateLocal(0.0f, -45.0f * time.DeltaTime, 0.0f);

			//Right
			obj4.get<Transform>().RotateLocal(0.0f, 45.0f * time.DeltaTime, 0.0f);
			obj5.get<Transform>().RotateLocal(0.0f, 45.0f * time.DeltaTime, 0.0f);

			//sub
			obj7.get<Transform>().RotateLocal(0.0f, 0.0f, 130.0f * time.DeltaTime);
